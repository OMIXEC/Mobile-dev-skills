#!/bin/bash
# Interactive install script for OpenCode
# Usage: curl -sL https://raw.githubusercontent.com/omixec/mobile-skills/main/install-opencode.sh | bash
...
print_header() {
    echo -e "${CYAN}╔══════════════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║           Mobile Skills Installer - OpenCode            ║${NC}"
    echo -e "${CYAN}╚══════════════════════════════════════════════════════════╝${NC}"
    echo
}
...
install_global() {
    ...
    echo -e "${YELLOW}Restart OpenCode to use the new skills.${NC}"
}

install_project() {
    local project_dir=$(pwd)
    local project_skills_dir="$project_dir/.opencode/skills"
...
install_custom() {
    ...
    if [ "$install_type" = "1" ]; then
        install_to_global "${selected_skills[@]}"
    else
        install_to_project "${selected_skills[@]}"
    fi
}
